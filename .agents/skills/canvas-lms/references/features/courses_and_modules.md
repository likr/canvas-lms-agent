...
### 1. View Courses
- **`GET /api/v1/courses`**: Lists the active courses for the authenticated user.
...
- **`GET /api/v1/courses/:id`**: Retrieves details of a specific course.
  - *Parameters*: `id` (Course ID).

### 2. Sections & Enrollments
- **`GET /api/v1/courses/:course_id/sections`**: Lists all sections in a course.
  - *Parameters*: `course_id`.
  - *Usage*: Essential for managing students grouped by sections.
- **`GET /api/v1/courses/:course_id/enrollments`**: Lists all enrollments in a course or section.
...
### 1. View Modules
- **`GET /api/v1/courses/:course_id/modules`**: Lists modules in a course.
...
- **`GET /api/v1/courses/:course_id/modules/:id`**: Retrieves details for a specific module.
  - *Parameters*: `course_id`, `id` (Module ID).

### 2. Manage Modules (Teacher Only)
- **`POST /api/v1/courses/:course_id/modules`**: Creates a new module.
  - *Parameters*: `course_id`, `module[name]`, `module[unlock_at]`, `module[position]`.
- **`PUT /api/v1/courses/:course_id/modules/:id`**: Updates an existing module.
...
### 1. View Pages
- **`GET /api/v1/courses/:course_id/pages`**: Lists wiki pages in a course.
  - *Parameters*: `course_id`.
- **`GET /api/v1/courses/:course_id/pages/:url_or_id`**: Retrieves the detailed content (HTML body) of a page.
...
- **`POST /api/v1/courses/:course_id/pages`**: Creates a new wiki page.
  - *Parameters*: `course_id`, `wiki_page[title]`, `wiki_page[body]`, `wiki_page[published]`.
- **`PUT /api/v1/courses/:course_id/pages/:url_or_id`**: Updates an existing wiki page.
...
### 1. Files
- **`GET /api/v1/courses/:course_id/files`**: Lists files uploaded to a course.
  - *Parameters*: `course_id`, `search_term` (Optional filter).
  - *Usage*: Use to locate lecture slides, PDFs, syllabus files, etc.

### 2. Rubrics
- **`GET /api/v1/courses/:course_id/rubrics`**: Lists rubrics associated with a course.
  - *Parameters*: `course_id`.
